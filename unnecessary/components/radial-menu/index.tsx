...
export default function RadialMenu() {
  ...
  const handleTouchEnd = useCallback((e: TouchEvent) => {
    ...
    if (isOpenRef.current) {
      ...
      if (activeIndexRef.current !== null && !isDisabledRef.current) {
        ...
        const angleDeg = (activeIndexRef.current * slice) - 90;
        const angleRad = (angleDeg * Math.PI) / 180;
        const radius = 100;
        
        const px = origin.x + Math.cos(angleRad) * radius + window.scrollX;
        const py = origin.y + Math.sin(angleRad) * radius + window.scrollY;

        triggerConfetti(px, py, item, int);
  ...
  }, [handleMouseDown, handleMouseMove, handleMouseUp, handleContextMenu, handleTouchStart, handleTouchMove, handleTouchEnd]);
