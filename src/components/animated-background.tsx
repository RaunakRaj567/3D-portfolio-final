...
const KeyboardScene = ({ maxDpr }: { maxDpr: number }) => {
  ...
  const hoveredKeycapRef = useRef<string | null>(null);
  ...
  const handleMouseHover = (e: SplineEvent) => {
    ...
    if (e.target.name === "body" || e.target.name === "platform") {
      if (hoveredKeycapRef.current) {
        playReleaseSound();
        hoveredKeycapRef.current = null;
      }
      ...
      if (hoveredKeycapRef.current !== e.target.name) {
        const skill = SKILLS[e.target.name as SkillNames];
        if (skill) {
          if (hoveredKeycapRef.current) playReleaseSound();
          playPressSound();
          hoveredKeycapRef.current = e.target.name;
        }
      }
    }
  };
