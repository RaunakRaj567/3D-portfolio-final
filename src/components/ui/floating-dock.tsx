...
export const FloatingDock = ({
  ...
  if (isMobile) {
    return (
      <div className={cn("flex flex-wrap gap-2 justify-center", mobileClassName)}>
        {items.map((item) => (
          <div
            key={item.title}
            className="h-10 w-10 rounded-full bg-secondary/40 border border-border/50 flex items-center justify-center relative group"
            title={item.title}
          >
            <div className="h-5 w-5 flex items-center justify-center text-foreground">{item.icon}</div>
          </div>
        ))}
      </div>
    );
